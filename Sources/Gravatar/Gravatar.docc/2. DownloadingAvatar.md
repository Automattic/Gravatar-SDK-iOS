...
## Providing a custom Image Cache

To integrate Gravatar with your own caching system, you can supply a custom cache implementation that conforms to the ImageCaching protocol.

To do so, define a type that adopts the ImageCaching protocol by implementing the required methods:

```swift
extension MyImageCache: ImageCaching {
    func setEntry(_ entry: Gravatar.CacheEntry?, for key: String) {
        // Store the cache entry for the given key
    }

    func getEntry(with key: String) -> Gravatar.CacheEntry? {
        // Retrieve the cache entry associated with the given key
    }
}
```
You can then initialize the avatar service with your custom cache:
```swift
// Create an avatar service instance with a custom image cache
let avatarService = Gravatar.AvatarService(cache: MyImageCache.shared)
```
This allows Gravatar to use your caching mechanism for storing and retrieving avatar images.

This custom cache implementation can also be utilized by the [UIImageView extension](https://automattic.github.io/Gravatar-SDK-iOS/gravatarui/documentation/gravatarui/uiimageviewextension/) and the SwiftUI [AvatarView](https://automattic.github.io/Gravatar-SDK-iOS/gravatarui/documentation/gravatarui/avatarviewarticle/) component.
