...
public typealias GravatarOpenAPIClientAPIChallengeHandler = (URLSession, URLSessionTask, URLAuthenticationChallenge) -> (
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.GravatarOpenAPIClient.\(UUID().uuidString)")
                }
...
extension Data {
    ...
    ///
    /// Rather than littering my code with calls to `dataUsingEncoding` to convert strings to Data, and then add that data to the Data, this wraps it in a nice
    /// convenient little extension to Data. This converts using UTF-8.
    ///
