...
final class MovableView: UIView, NSSecureCoding {
    ...
    var transformations: ViewTransformations {
        ViewTransformations(position: position, scale: scale, rotation: rotation)
    }
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        let view = aDecoder.decodeObject(of: [StylableImageView.self], forKey: CodingKeys.innerView.rawValue)
        ...
        if let imageView = view as? StylableImageView {
            innerView = imageView
        } else {
            innerView = StylableImageView(id: UUID().uuidString, image: nil)
        }
